...
public client class LibraryManagementBookInfoCaller {
    private grpc:Caller caller;
    ...
    isolated remote function sendBookInfo(bookInfo response) returns grpc:Error? {
        return self.caller->send(response);
    }

    isolated remote function sendContextBookInfo(ContextBookInfo response) returns grpc:Error? {
        return self.caller->send(response);
    }
